...
async function testActivitiesAPI() {
  ...
  try {
    ...
    const activitiesResponse = await axios.get(`${BASE_URL}/auth/activities`, {
    ...
    const semesterResponse = await axios.get(`${BASE_URL}/auth/activities?semester=hoc_ky_1`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    console.log('✅ Semester filter working:', semesterResponse.data.data.total, 'activities');

    // Test status filter
    const statusResponse = await axios.get(`${BASE_URL}/auth/activities?status=da_tham_gia`, {
      headers: { Authorization: `Bearer ${token}` }
    });
