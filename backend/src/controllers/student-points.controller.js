...
class StudentPointsController {
  ...
  async getPointsSummary(req, res) {
    try {
      ...
      // Lấy các đăng ký hoạt động đã được phê duyệt - match class management logic exactly
      const registrations = await prisma.dangKyHoatDong.findMany({
        where: {
          sv_id: sinhVien.id,
          trang_thai_dk: {
            in: ['da_tham_gia', 'da_duyet'] // Count both participated and approved
          }
        },
        include: {
          hoat_dong: {
  ...
  async getFilterOptions(req, res) {
    try {
      ...
      // Lấy danh sách học kỳ có dữ liệu - match main summary logic
      ...
      // Lấy danh sách năm học có dữ liệu - match main summary logic
