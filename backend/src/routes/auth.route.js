...
async function ensureDemoUsersIfNeeded() {
  ...
  try {
    ...
    const getRoleId = async (name, mo_ta) => {
      const found = roles.find(r => r.ten_vt === name) || await prisma.vaiTro.create({ data: { ten_vt: name, mo_ta } });
      return found.id || (await prisma.vaiTro.findUnique({ where: { ten_vt: name } })).id;
    };
    const rADMIN = await getRoleId('ADMIN', 'Quản trị viên hệ thống');
    const rGV = await getRoleId('GIANG_VIEN', 'Giảng viên');
    const rLT = await getRoleId('LOP_TRUONG', 'Lớp trưởng');
    const rSV = await getRoleId('SINH_VIEN', 'Sinh viên');
    ...
    await upsert('gv001', 'nguyenvana@dlu.edu.vn', 'Nguyễn Văn A', 'Teacher@123', rGV);
    await upsert('lt001', 'tranvanc@dlu.edu.vn', 'Trần Văn C', 'Monitor@123', rLT);
    await upsert('2021003', '2021003@dlu.edu.vn', 'Lê Minh Tuấn', 'Student@123', rSV);
  } catch (_) {}
}
...
router.get('/roles', async (req, res) => {
  try {
    ...
    sendResponse(res, 200, ApiResponse.success(data, 'Roles'));
  } catch (error) {
    logError('Get roles error', error);
    sendResponse(res, 500, ApiResponse.error('Không lấy được danh sách vai trò'));
  }
});
...
router.post('/demo-ensure', async (req, res) => {
  try {
    if ((config.nodeEnv || 'development') !== 'development') {
      return sendResponse(res, 403, ApiResponse.forbidden('Chỉ dùng trong môi trường development'));
    }

    // Tạo vai trò nếu thiếu
    const roles = await prisma.vaiTro.findMany();
    const roleMap = new Map(roles.map(r => [r.ten_vt, r]));
    async function ensureRole(name, mo_ta) {
      if (!roleMap.has(name)) {
        const r = await prisma.vaiTro.create({ data: { ten_vt: name, mo_ta } });
        roleMap.set(name, r);
      }
      return roleMap.get(name);
    }
    const rADMIN = await ensureRole('ADMIN', 'Quản trị viên hệ thống');
    const rGV = await ensureRole('GIANG_VIEN', 'Giảng viên');
    const rLT = await ensureRole('LOP_TRUONG', 'Lớp trưởng');
    const rSV = await ensureRole('SINH_VIEN', 'Sinh viên');

    // Upsert người dùng
    async function upsertUser(ten_dn, email, ho_ten, password, vai_tro_id) {
      const hashed = await bcrypt.hash(password, 10);
      const existing = await prisma.nguoiDung.findUnique({ where: { ten_dn } });
      if (existing) return existing;
      return prisma.nguoiDung.create({ data: { ten_dn, email, ho_ten, mat_khau: hashed, vai_tro_id, trang_thai: 'hoat_dong' } });
    }

    await upsertUser('admin', 'admin@dlu.edu.vn', 'Quản Trị Viên', 'Admin@123', rADMIN.id);
    await upsertUser('gv001', 'nguyenvana@dlu.edu.vn', 'Nguyễn Văn A', 'Teacher@123', rGV.id);
    const lt = await upsertUser('lt001', 'tranvanc@dlu.edu.vn', 'Trần Văn C', 'Monitor@123', rLT.id);
    await upsertUser('2021003', '2021003@dlu.edu.vn', 'Lê Minh Tuấn', 'Student@123', rSV.id);

    return sendResponse(res, 200, ApiResponse.success(true, 'Đã đảm bảo 4 tài khoản demo'));
  } catch (error) {
    logError('Demo ensure error', error);
    return sendResponse(res, 500, ApiResponse.error('Không thể tạo tài khoản demo'));
  }
});
