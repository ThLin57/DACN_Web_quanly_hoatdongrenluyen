...
# =============================================================================
# Tải backup từ EC2 production và restore vào DB dev local
# =============================================================================
...
try {
    $result = Invoke-Expression "$sshCmd `"$createBackupCmd`"" 2>&1
...
try {
    $result = Invoke-Expression "$sshCmd `"rm -f /tmp/$backupFile`"" 2>&1
    Write-Host "✓ Đã xóa backup tạm trên EC2" -ForegroundColor Green
} catch {
    Write-Host "⚠ Không thể xóa backup trên EC2 (không ảnh hưởng)" -ForegroundColor Yellow
}
...
Write-Host ""
Write-Host "BƯỚC 4: RESTORE BACKUP VÀO DB DEV" -ForegroundColor Yellow
Write-Host "----------------------------------------" -ForegroundColor Yellow
...
docker exec -it dacn_db psql -U admin -c "DROP DATABASE IF EXISTS \"Web_QuanLyDiemRenLuyen\";" postgres 2>$null
docker exec -it dacn_db psql -U admin -c "CREATE DATABASE \"Web_QuanLyDiemRenLuyen\";" postgres

Write-Host "Đang restore dữ liệu..." -ForegroundColor Cyan
docker exec -it dacn_db pg_restore -U admin -d Web_QuanLyDiemRenLuyen -v /tmp/restore.dump 2>&1 | Out-Null
