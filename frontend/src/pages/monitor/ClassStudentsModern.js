...
export default function ClassStudents() {
  ...
  const stats = {
    ...
    topPerformers: students.filter(s => s.totalPoints >= 80).length,
    needsSupport: students.filter(s => s.totalPoints < 30).length
  };
        ...
        <div className="p-6 relative z-10">
          ...
          <div className="flex items-start gap-4 mb-4">
            <div className="flex-shrink-0">
              <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${getProgressColor(student.totalPoints)} flex items-center justify-center text-white font-bold text-2xl shadow-lg ring-4 ring-white`}>
                {student.nguoi_dung.ho_ten.split(' ').pop().charAt(0)}
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-gradient-to-br from-amber-400 to-orange-500 rounded-2xl p-6 text-white shadow-lg hover:shadow-xl transition-shadow">
            ...
            <div className="text-3xl font-bold mb-1">{stats.needsSupport}</div>
            <div className="text-amber-100 text-sm font-medium">Cần hỗ trợ</div>
          </div>
        </div>
