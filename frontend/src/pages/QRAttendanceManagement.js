...
export default function QRAttendanceManagement() {
  ...
  const loadQrForActivity = async (activityId) => {
    try {
      // Guard on client: only allow QR creation on start date or later
      const act = activities.find(x => x.id === activityId) || selectedActivity;
      const now = new Date();
      const start = act ? new Date(act.ngay_bd) : null;
      const end = act ? new Date(act.ngay_kt) : null;
      const isSameDay = (a,b)=> a.getFullYear()===b.getFullYear() && a.getMonth()===b.getMonth() && a.getDate()===b.getDate();
      if (start && now < start) {
        setError('Chỉ tạo QR vào ngày bắt đầu hoạt động');
        return;
      }
      if (end && isSameDay(now, end)) {
        setError('Đã hết hạn điểm danh trong ngày kết thúc');
        return;
      }
      const res = await http.get(`/activities/${activityId}/qr?image=1`);
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            {selectedActivity ? (
              <div className="space-y-3">
                ...
                <div className="space-y-3">
                  <button
                    onClick={() => loadQrForActivity(selectedActivity.id)}
                    className="w-full bg-purple-600 text-white py-2 px-4 rounded-lg flex items-center justify-center gap-2 hover:bg-purple-700 disabled:opacity-50"
                    disabled={selectedActivity && (new Date(selectedActivity.ngay_bd) > new Date() || (function(){ const n=new Date(); const e=new Date(selectedActivity.ngay_kt); return n.getFullYear()===e.getFullYear() && n.getMonth()===e.getMonth() && n.getDate()===e.getDate(); })())}
                  >
                    <QrCode className="w-4 h-4" /> Tạo mã QR điểm danh
                  </button>
                  {qrDataUrl && (
                    <div className="border rounded-lg p-3 text-center">
                      <img src={qrDataUrl} alt="QR attendance" className="mx-auto w-56 h-56" />
                      <div className="mt-3 flex items-center justify-center gap-2">
                        <button onClick={() => loadQrForActivity(selectedActivity.id)} className="px-3 py-1 text-sm bg-gray-100 rounded hover:bg-gray-200 flex items-center gap-1">
                          <RefreshCw className="w-4 h-4" /> Làm mới
                        </button>
                        <a href={qrDataUrl} download={`qr-${selectedActivity.id}.png`} className="px-3 py-1 text-sm bg-gray-100 rounded hover:bg-gray-200">Tải ảnh</a>
                      </div>
                    </div>
                  )}
                </div>
