...
export default function Login() {
  ...
  React.useEffect(function loadDemo() {
    let mounted = true;
    const defaults = [
      { username: 'admin', email: 'admin@dlu.edu.vn', password: 'Admin@123' },
      { username: 'gv001', email: 'nguyenvana@dlu.edu.vn', password: 'Teacher@123' },
      { username: 'lt001', email: 'tranvanc@dlu.edu.vn', password: 'Monitor@123' },
      { username: '2021003', email: '2021003@dlu.edu.vn', password: 'Student@123' },
    ];
    // Hiển thị sẵn 4 tài khoản demo mặc định để người dùng có thể dùng ngay
    try { if (mounted) setDemoAccounts(defaults); } catch (_) {}
    http
  ...
  async function loginWithDemo(username, password) {
    ...
    try {
  var res = await http.post('/auth/login', { maso: String(username || '').trim().toLowerCase(), password: password });
      var data = res.data?.data || res.data;
      var token = data?.token || data?.data?.token;
      if (token) {
        var user = data?.user || null;
