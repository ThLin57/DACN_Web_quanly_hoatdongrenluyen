  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <img src={logo} alt="Logo hệ thống quản lý hoạt động sinh viên" className="h-8 w-auto" />
              </div>
              <div className="hidden md:block ml-4">
                <h1 className="text-xl font-bold text-gray-800">Hệ Thống Quản Lý Hoạt Động</h1>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                <button className="flex items-center space-x-2">
                  <img src="https://via.placeholder.com/32x32/3B82F6/FFFFFF?text=U" alt="Ảnh đại diện người dùng" className="h-8 w-8 rounded-full" />
      ...
      <div className="flex">
        ...
        <aside className="hidden md:flex w-64 text-white" style={{background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', minHeight: 'calc(100vh - 64px)'}}>
          <nav className="mt-8 w-full">
            <div className="px-4">
              <h2 className="text-sm font-semibold uppercase tracking-wider text-blue-100 mb-4">Menu Chính</h2>
              <ul className="space-y-2">
                <li>
                  <Link
                    to="/"
                    className={`flex items-center px-4 py-2 rounded-lg ${isActive('/') ? 'bg-blue-800' : 'hover:bg-blue-700'}`}
                  >
                ...
                <li>
                  <Link
                    to="/profile"
                    className={`flex items-center px-4 py-2 rounded-lg ${isActive('/profile') ? 'bg-blue-800' : 'hover:bg-blue-700'}`}
                  >
                ...
                <li>
                  <a href="#" className="flex items-center px-4 py-2 hover:bg-blue-700 rounded-lg">
                ...
                <li className="relative" ref={settingsRef}>
                  <button onClick={() => setShowSettings(!showSettings)} className="flex items-center px-4 py-2 hover:bg-blue-700 rounded-lg w-full text-left">
                    <i className="fas fa-cog mr-3"></i>
                    <span>Cài đặt</span>
                    <i className={`fas fa-chevron-right text-xs ml-auto transition-transform ${showSettings ? 'rotate-90' : ''}`}></i>
                  </button>
                  {showSettings && (
                    <ul className="pl-4 mt-1 space-y-1">
                      <li>
                        <button onClick={() => { openChangePwd(); setShowSettings(false); }} className="flex items-center px-4 py-2 text-blue-100 hover:bg-blue-700 rounded-lg w-full text-left text-sm">
                          <i className="fas fa-key mr-3 w-4 text-center"></i>
                          <span>Đổi mật khẩu</span>
                        </button>
                      </li>
                      <li>
                        <button onClick={() => { toggleTheme(); setShowSettings(false); }} className="flex items-center px-4 py-2 text-blue-100 hover:bg-blue-700 rounded-lg w-full text-left text-sm">
                ...
                <li>
                  <button onClick={handleLogout} className="flex items-center px-4 py-2 hover:bg-blue-700 rounded-lg w-full text-left">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-2">
        <div className="flex justify-around">
          ...
          <button onClick={handleLogout} className="flex flex-col items-center py-2 text-gray-600">
      ...
      {showMobileSettings && (
        <div className="fixed inset-0 z-50 flex items-end">
          ...
          <div className="relative bg-white rounded-t-xl shadow-2xl w-full p-4">
            ...
            <h3 className="text-lg font-semibold text-gray-900 text-center mb-4">Cài đặt</h3>
            <ul className="space-y-2">
              <li>
